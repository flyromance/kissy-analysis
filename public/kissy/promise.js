...
KISSY.add("promise", [], function (S) {
  ...
  Promise.prototype = {
    ...
    fin: function (callback) {
      ...
      var self = this,
        ...
        promiseToHandle =
          fulfilled || rejected ? self.then(fulfilled, rejected) : self;
  ...
  function Reject(reason) {
    ...
    if (self[PROMISE_VALUE] instanceof Promise) {
      logger.error(
        "assert.not(this.__promise_value instanceof promise) in Reject constructor"
      );
    }
    return self;
  }
  ...
  function isResolved(obj) {
    return (
      ...
      (!isPromise(obj[PROMISE_VALUE]) || isResolved(obj[PROMISE_VALUE]))
    );
  }
